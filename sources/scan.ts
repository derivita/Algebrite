...
import { bignum_scan_float, bignum_scan_integer, push_integer } from './bignum';
  ...
  if (
    ...
    if (DEBUG) {
      console.log(
        '... adding symbol: ' +
          theSymbol +
          ' to the set of the symbols right of assignment'
      );
    }
    let prefixVar = '';
  ...
  if (
    ...
    if (DEBUG) {
      console.log(
        '... adding symbol: ' +
          theSymbol +
          ' to the set of the symbols left of assignment'
      );
    }
    let prefixVar = '';
    for (let i = 1; i < functionInvokationsScanningStack.length; i++) {
