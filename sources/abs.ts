...
export function abs() {
  const theArgument = top();
  if (DEBUG_ABS) {
    console.trace('>>>>  ABS of ' + theArgument);
  }
  numerator();
  if (DEBUG_ABS) {
    console.log('ABS numerator ' + top());
  }
  absval();
  if (DEBUG_ABS) {
    console.log('ABSVAL numerator: ' + top());
  }

  push(theArgument);
  denominator();
  if (DEBUG_ABS) {
    console.log('ABS denominator: ' + top());
  }
  absval();
  if (DEBUG_ABS) {
    console.log('ABSVAL denominator: ' + top());
  }

  divide();
  if (DEBUG_ABS) {
    console.log('ABSVAL divided: ' + top());
  }
...
export function absval() {
  ...
  if (car(p1) === symbol(POWER) && ispositivenumber(caddr(p1))) {
    if (DEBUG_ABS) {
      console.log(
        ' abs: ' + p1 + ' is something to the power of a positive number'
      );
    }
    push(cadr(p1));
    abs();
...
function absval_tensor(p1: Tensor) {
  if (p1.tensor.ndim !== 1) {
