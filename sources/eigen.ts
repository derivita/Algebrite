...
import { push_symbol, set_binding, usr_symbol } from '../runtime/symbol';
...
export function Eval_eigenvec(p1: U) {
  ...
  let [_, p3] = eigen(EIGENVEC, p1);

  push(p3);
}

function EIG_check_arg(p1: U): p1 is Tensor<Double> {
  push(cadr(p1));
  Eval();
  yyfloat();
  Eval();
