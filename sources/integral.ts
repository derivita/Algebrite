...
import { push_symbol, symnum } from '../runtime/symbol';
import { equal } from '../sources/misc';
import { add } from './add';
import { pop_integer, push_double } from './bignum';
...
export function integral() {
  ...
  if (car(p1) === symbol(ADD)) {
    integral_of_sum(p1, p2);
  } else if (car(p1) === symbol(MULTIPLY)) {
    integral_of_product(p1, p2);
  } else {
    integral_of_form(p1, p2);
  }
  p1 = pop();
...
function integral_of_sum(p1: U, p2: U) {
  p1 = cdr(p1);
  push(car(p1));
  push(p2);
  integral();
...
function integral_of_product(p1: U, p2: U) {
  push(p1);
  push(p2);
  partition();
...
function integral_of_form(p1: U, p2: U) {
