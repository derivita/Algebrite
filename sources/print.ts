...
export function collectLatexStringFromReturnValue(p: BaseAtom): string {
  ...
  if (DEBUG) {
    console.log(
      'emttedString from collectLatexStringFromReturnValue: ' +
...
function print_base_of_denom(BASE: BaseAtom): string {
  ...
  if (
    isfraction(BASE) ||
    car(BASE) === symbol(ADD) ||
    car(BASE) === symbol(MULTIPLY) ||
    car(BASE) === symbol(POWER) ||
    lessp(BASE as U, defs.zero)
  ) {
...
function print_expo_of_denom(EXPO: BaseAtom): string {
  ...
  if (
    isfraction(EXPO) ||
    car(EXPO) === symbol(ADD) ||
    car(EXPO) === symbol(MULTIPLY) ||
    car(EXPO) === symbol(POWER)
  ) {
    accumulator += print_char('(');
...
function sign_of_term(p: BaseAtom): string {
  ...
  if (
    ...
    lessp(cadr(p), defs.zero)
  ) {
    accumulator += '-';
  } else if (isNumericAtom(p) && lessp(p, defs.zero)) {
    accumulator += '-';
  } else {
    accumulator += '+';
  }
...
function print_power(base: BaseAtom, exponent: BaseAtom) {
  ...
  if (defs.printMode === PRINTMODE_LATEX && isplusone(exponent)) {
    ...
    if (isadd(base) || isnegativenumber(base)) {
      ...
      isNumericAtom(base) &&
      (lessp(base, defs.zero) || isfraction(base))
    ) {
      accumulator += print_str('(');
    ...
    if (defs.printMode === PRINTMODE_LATEX) {
      ...
      iscons(exponent) ||
      isfraction(exponent) ||
      (isNumericAtom(exponent) && lessp(exponent, defs.zero))
    ) {
      accumulator += print_str('(');
      accumulator += print_expr(exponent);
      accumulator += print_str(')');
...
function print_factor(p: BaseAtom, omitParens = false): string {
  // debugger
  let accumulator = '';
  if (isNumericAtom(p)) {
  ...
  if (car(p) === symbol(FUNCTION)) {
    ...
    if (!defs.codeGen) {
      ...
      if (DEBUG) {
        console.log(
          'emittedString from print_factor ' +
            defs.stringsEmittedByUserPrintouts
        );
      }
