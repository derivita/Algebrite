...
class Defs {
  ...
  public expanding = 0;
  public evaluatingAsFloats = 0;
  public evaluatingPolar = 0;
  public esc_flag = 0;
  public trigmode = 0;
  ...
  public zero!: Num;
  public one!: Num;
  public one_as_double!: Double;
  public imaginaryunit!: U;
...
export function isadd(p: BaseAtom): boolean {
  return car(p) === symbol(ADD);
}

export function ismultiply(p: BaseAtom): boolean {
  return car(p) === symbol(MULTIPLY);
}

export function ispower(p: BaseAtom): boolean {
  return car(p) === symbol(POWER);
}

export function isfactorial(p: BaseAtom): boolean {
...
export function isidentitymatrix(p): boolean {
...
export function reset_after_error() {
  ...
  draw_flag = 0;
  defs.frame = TOS;
  defs.evaluatingAsFloats = 0;
  defs.evaluatingPolar = 0;
}
