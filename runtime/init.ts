...
import { push_double, push_integer, push_rational } from '../sources/bignum';
...
import { push_symbol, std_symbol } from './symbol';
let init_flag = 0;
...
function defineSomeHandyConstants() {
  push_integer(0);
  defs.zero = pop() as Num; // must be untagged in gc

  push_integer(1);
  defs.one = pop() as Num; // must be untagged in gc

  push_double(1.0);
  defs.one_as_double = pop() as Double;
  ...
  defs.imaginaryunit = pop(); // must be untagged in gc
}
